...
// update 3: adicionado modelo probabilístico com rankeamento de todos os documentos (30/04/2018)
...
if size(qTokens, 'r') > 1 then // question has more than one word
    printf("\n")
    printf("Documents that answer the conjunctive request (AND):")
    disp(documentsAnd)
    for i=1:numberOfDocuments
    ...
    printf("\n")
    printf("Documents that answer the disjunctive request (OR):")
    disp(documentsOr)
    for i=1:numberOfDocuments
...
// Calculating avg_doclen
avg_doclen = 0
for i=1:numberOfDocuments
    lenDocument = size(tokens(M(i), separators), 'r')
    for k=1:size(stopwords, 'r')
        lenDocument = lenDocument - length(find(tokens(M(j)) == stopwords(k))) // removing stopwords
    end
    avg_doclen = avg_doclen + lenDocument
end
avg_doclen = avg_doclen / numberOfDocuments

//Bij = ((k1 + 1) * fij) / k1 * ( (1 - b) + b*(len(dj)/avg_doclen)) + fij

// Calculating the ranks for each document
documentRank = zeros(numberOfDocuments, 1)
for j=1:numberOfDocuments
    lenDocument = size(tokens(M(j), separators), 'r')
    for k=1:size(stopwords, 'r')
        lenDocument = lenDocument - length(find(tokens(M(j)) == stopwords(k))) // removing stopwords
    end

    for i=1:numberTokens
        if find(tokens(q, separators) == myTokens(i)) ~= [] then
            // calculating ni for each term ki
            // ni = number of documents that have the term i
            ni = 0
            for k=1:numberOfDocuments
                if incidenceMatrix(i,k) > 0 then
                    ni = ni + 1
                end
            end
            // calculate second part of the ranking formula
            term2 = log2( (numberOfDocuments - ni + 0.5) / (ni + 0.5) )
            
            
            fij = incidenceMatrix(i,j)
            // calculate Bij for each term i in document j
            if fij ~= 0 then
                Bij = ( (k1 + 1) * fij) / k1 * ( (1 - b) + b * (lenDocument / avg_doclen) ) + fij
                documentRank(j,1) = documentRank(j,1) + (Bij * term2)
            end
        end
    end
end
...
//sorting
sortedDocumentRank = gsort(documentRank)
documentsIndexes = []
for i=1:numberOfDocuments
    for j=1:numberOfDocuments
       if (documentRank(j) == sortedDocumentRank(i)) then
           documentsIndexes = [documentsIndexes; j]
       end
    end
end
...
// now showing on screen
printf("\n---------------------RANKED DOCUMENTS FOR YOUR SEARCH (PROBABILISTIC MODEL)---------------------\n\n")
printf("______________________________________________________\n\n")
printf("Your search was: ")
printf("   %s\n\n", q)
printf("______________________________________________________\n\n")
...
for i=1:size(documentsIndexes, 'r')
    printf("RANK: %f \t\t DOCUMENT NUMBER: %d \n\n", sortedDocumentRank(i), documentsIndexes(i))
    printf("%s", M(documentsIndexes(i)))
    printf("\n\n")
    printf("______________________________________________________\n\n")
end

// ----------------- PROBABILISTIC MODEL END --------------------
